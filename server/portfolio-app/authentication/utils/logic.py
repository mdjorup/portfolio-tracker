...
db_client = MongoClient(
    f"mongodb+srv://admin:{db_pw}@cluster0.bo9qn.mongodb.net/?retryWrites=true&w=majority"
)
...
def register(body):
    ...
    try:
        ...
        return build_response(
            500, {"error": "ServerError", "message": "Unable to register user to DB"}
        )
    ...
    return build_response(
        200, {"jwt": jwt_token, "message": "Successfully registered user"}
    )
...
def login(body):
    ...
    if not bcrypt.checkpw(password, encrypted_pw):
        return build_response(
            400, {"error": "Unauthorized", "message": "Incorrect password"}
        )

    jwt_token = generate_jwt(username)

    return build_response(200, {"jwt": jwt_token, "message": "Successfully Logged In"})
...
def verify(token):
    ...
    try:
        ...
        return build_response(200, {"status": "Success", "message": "Valid JWT"})
        ...
        return build_response(
            401,
            {"error": "InvalidTokenError", "message": "Invalid Token. Log In Again"},
        )
